...
# Data-temperature map (classification follows runtime access frequency):
# Hot — checked for every model/tool response: protocol markers, finish
# reasons, tool-call admission, and the single-validation policy.
# Warm — read or updated during active Session work: state windows, cache
# bounds, resume projections, and AgentState serialization limits.
# Cold — consulted mainly during compaction, recovery, or observability:
# durable history/registry bounds and scalar event/error projection limits.

# Hot: DeepSeek/tool protocol boundaries.
...
# Hot: one-validation policy shared by execution admission and completion evidence.
...
# Warm: in-memory state windows.  Older evidence is summarized or kept in a
...
# Warm: AgentState is a model-adjacent projection rather than an unbounded
...
# Cold: bounded metadata retained when warm state is compacted.
MAX_TOOL_HISTORY_CAPABILITIES = 64
MAX_ARTIFACT_REGISTRY_ITEMS = 128
MAX_INTENT_JOURNAL_ENTRIES = 512

# Cold: scalar/error projection limits shared by capability and recovery paths.
